  ...
  return (
    <div className="flex h-full w-full bg-background overflow-hidden font-sans text-text-primary">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        <header className={cn(
          "border-b border-border bg-surface/80 backdrop-blur-md flex items-center shrink-0 z-40",
          isMobileMockup ? "h-11 px-3 justify-between" : "h-14 md:h-16 px-4 md:px-6 justify-between md:justify-end"
        )}>
            {isMobileMockup && (
              <div className="flex items-center gap-2 min-w-0">
                <div className="w-5 h-5 bg-blue-600 rounded flex items-center justify-center font-black text-[10px] text-white shrink-0 shadow-sm">K</div>
                <span className="text-xs font-bold text-text-primary truncate">Quartier Neubau Süd</span>
              </div>
            )}
            <div className="flex items-center gap-1.5 px-2.5 py-1 bg-brand-500/10 border border-brand-500/20 rounded-full shrink-0">
              <Sparkles size={12} className="text-brand-500" />
              <span className="text-[9px] font-bold text-brand-500 uppercase">Live OS</span>
            </div>
        </header>

        {/* MOBILE NAVIGATION (Wischbar / Horizontal scrollbar) */}
        <div className={cn(
          "flex items-center gap-1.5 px-3 py-2 bg-surface/95 backdrop-blur-xl border-b border-border overflow-x-auto hide-scrollbar shrink-0 w-full z-40 shadow-sm sticky top-0",
          !isMobileMockup && "md:hidden"
        )}>
          {mobileNavItems.map(item => {
            ...
            return (
              <button 
                ...
                onClick={() => setActiveTab(item.id)}
                className={cn(
                  "flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-bold whitespace-nowrap transition-all duration-200 border shrink-0 cursor-pointer", 
