...
export default function FinanceTab({ addToast, setShowExpenseModal, setShowInvoiceModal, setShowQuoteModal }: FinanceTabProps) {
  ...
  const handleSaveToCloud = async (blob: Blob) => {
    ...
    try {
      ...
      setOpCostReceipts([]);
      setOpCostData({ category: 'Fremdleistungen & Subunternehmer', description: '', amount: '', date: new Date().toISOString().split('T')[0] });
    } catch (error) {
    ...
    <div className="w-full font-sans pb-12" style={{ color: '#000000', backgroundColor: '#ffffff' }}>
      ...
      <div className="flex justify-between items-start mt-4 mb-16">
        <div className="flex flex-col w-1/2 pt-4">
          <h1 className="text-4xl font-black uppercase tracking-widest mb-2" style={{ color: '#a855f7' }}>
            BUCHUNG
          </h1>
          <div className="text-xl font-bold uppercase tracking-widest mt-1" style={{ color: '#6b7280' }}>
            EXTERNER BELEG
          </div>
        </div>
        ...
        <div className="w-1/2 flex justify-end">
           <table className="text-sm text-right">
             <tbody>
               <tr>
                 <td className="pr-4 py-1 font-medium" style={{ color: '#6b7280' }}>{t('invoice_date')}:</td>
                 <td className="font-bold" style={{ color: '#000000' }}>{new Date(opCostData.date).toLocaleDateString('de-CH')}</td>
               </tr>
               <tr>
                 <td className="pr-4 py-1 font-medium" style={{ color: '#6b7280' }}>{t('recorded_date')}:</td>
                 <td className="font-bold" style={{ color: '#000000' }}>{new Date().toLocaleDateString('de-CH')}</td>
               </tr>
             </tbody>
           </table>
        </div>
      ...
      <table className="w-full text-sm text-left mb-16" style={{ borderCollapse: 'collapse' }}>
        <thead style={{ borderBottom: '2px solid #000000', color: '#000000' }}>
          <tr>
            <th className="py-3 px-2 font-bold w-48">{t('category')}</th>
            <th className="py-3 px-2 font-bold">{t('company_purpose')}</th>
            <th className="py-3 px-2 font-bold text-right w-36 whitespace-nowrap">{t('amount')} (CHF)</th>
          </tr>
        </thead>
        <tbody>
          <tr className="break-inside-avoid" style={{ borderBottom: '1px solid #e5e7eb' }}>
            <td className="py-4 px-2 align-top pr-4">
              <span className="px-2 py-1 rounded text-xs font-bold uppercase tracking-wider" style={{ color: '#4b5563', backgroundColor: '#f3f4f6' }}>
                {opCostData.category}
              </span>
            </td>
            <td className="py-4 px-2 align-top whitespace-pre-wrap font-bold" style={{ color: '#000000' }}>
              {opCostData.description || '-'}
            </td>
            <td className="py-4 px-2 text-right font-bold align-top whitespace-nowrap" style={{ color: '#a855f7' }}>
              {formatCHF(Number(opCostData.amount))}
            </td>
          </tr>
        </tbody>
      </table>
      ...
      {opCostReceipts.length > 0 ? (
        <div className="break-before-page pt-8">
          <h3 className="text-lg font-bold mb-6 uppercase tracking-widest pb-2" style={{ borderBottom: '1px solid #a855f7', color: '#a855f7' }}>
            Original Beleg
          </h3>
          <div className="grid grid-cols-2 gap-4">
            {opCostReceipts.map((url, i) => (
        ...
        <div className="pt-8 text-center text-sm font-medium italic" style={{ color: '#9ca3af' }}>
          Kein Originalbeleg angehängt.
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300 text-text-primary">
                ...
                {quotes.map(quote => (
                  <tr key={quote.id} className="hover:bg-white/[0.02] group">
                    <td className="px-4 py-2">
                      <div className="font-bold text-xs truncate max-w-[150px]">{quote.description || quote.client || t('quote')}</div>
                    </td>
                    ...
                    <td className="px-4 py-2 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <select value={quote.status} onChange={(e) => handleUpdateStatus(quote.id, e.target.value)} className={cn("px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider border outline-none cursor-pointer appearance-none", quote.status === 'Angenommen' || quote.status === 'Approved' ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" : "bg-blue-500/10 text-blue-500 border-blue-500/20")}>
                          <option value="Offen" className="bg-surface">Offen</option>
                          <option value="Angenommen" className="bg-surface">Angenommen</option>
                          <option value="Abgelehnt" className="bg-surface">Abgelehnt</option>
                        </select>
              ...
              <tbody className="divide-y divide-border/30">
                {invoices.map(inv => (
                  <tr key={inv.id} className="hover:bg-white/[0.02] group">
                    <td className="px-4 py-2">
                      <div className="font-bold text-xs truncate max-w-[150px]">{inv.description || inv.client || 'Rechnung'}</div>
                    </td>
                    ...
                    <td className="px-4 py-2 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <select value={inv.status} onChange={(e) => handleUpdateStatus(inv.id, e.target.value)} className={cn("px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider border outline-none cursor-pointer appearance-none", inv.status === 'Bezahlt' || inv.status === 'paid' ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" : "bg-orange-500/10 text-orange-500 border-orange-500/20")}>
                          <option value="Offen" className="bg-surface">Offen</option>
                          <option value="Bezahlt" className="bg-surface">Bezahlt</option>
                        </select>
              ...
              <tbody className="divide-y divide-border/30">
                {expenses.map(exp => (
                  <tr key={exp.id} className="hover:bg-white/[0.02] group">
                    <td className="px-4 py-2">
                      <div className="font-bold text-xs truncate max-w-[150px]">{exp.description || exp.category || t('expenses')}</div>
                    </td>
              ...
              <tbody className="divide-y divide-border/30">
                {operatingCosts.map(op => (
                  <tr key={op.id} className="hover:bg-white/[0.02] group">
                    <td className="px-4 py-2">
                      <div className="font-bold text-xs truncate max-w-[150px]">{op.description || op.category || 'Kosten'}</div>
                    </td>
          ...
          <div className="bg-surface border border-border rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-4 border-b border-border/50 flex items-center justify-between shrink-0">
              <h3 className="font-bold text-lg flex items-center gap-2">
                <Landmark className="text-purple-500"/> {t('record_ext_cost')}
              </h3>
              {isAnalyzingAI && (
                <span className="flex items-center gap-2 px-3 py-1 bg-purple-500/10 text-purple-500 text-xs font-bold rounded-full animate-pulse border border-purple-500/20 ml-4">
                  <Sparkles size={12} /> {t('analyzing_ai')}
                </span>
              )}
      ...
      {/* Universal PDF Studio für Kassenbons */}
      <UniversalPDFStudio 
