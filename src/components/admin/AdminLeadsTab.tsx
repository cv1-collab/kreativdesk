    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="bg-surface border border-border p-6 rounded-3xl shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {isLoading ? (
            <div className="col-span-full text-center py-12 text-text-muted">Lade Leads...</div>
          ) : leads.length === 0 ? (
            <div className="col-span-full text-center py-12 text-text-muted">Keine neuen Leads vorhanden.</div>
          ) : (
            leads.map(lead => (
              <div key={lead.id} className="bg-background border border-border/50 p-5 rounded-2xl flex flex-col justify-between gap-4">
                <div>
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className="px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider bg-blue-500/10 text-blue-500 border border-blue-500/20">
                      {lead.status || 'Neu'}
                    </span>
                  </div>
                ...
                <div className="flex justify-between items-center border-t border-border/50 pt-3">
                  ...
                  >
                    <CheckCircle size={14} /> Als Erledigt markieren
                  </button>
                  <button 
                    onClick={() => handleDelete(lead.id)}
                    className="text-text-muted hover:text-red-500 p-1.5 rounded-lg"
                  >
                    <Trash2 size={16} />
                  </button>
                </div>
